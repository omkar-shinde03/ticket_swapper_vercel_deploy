...
export const TicketVerificationSystem = ({ onVerificationComplete, initialData = {} }) => {
  ...
  /**
   * Verify ticket with specific bus operator API
   * @param {string} pnr 
  ...
  const verifyWithOperatorAPI = async (pnr, operatorId, additionalData = {}) => {
    ...
    // In a real implementation, this would call the actual operator API
    // For now, we'll simulate the API call with enhanced mock data
    const response = await fetch('/api/verify-ticket', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-KEY': process.env.REACT_APP_TICKET_VERIFICATION_KEY || 'demo-key'
      },
      body: JSON.stringify({
        pnr: pnr.toUpperCase(),
        operator: operatorId,
  ...
  const advancedPNRValidation = async (pnr, operatorId, additionalData) => {
    ...
    // Enhanced mock validation with different operator patterns
    ...
    // Enhanced mock responses based on PNR patterns
    if (cleanPNR.startsWith('TEST') || cleanPNR.startsWith('DEMO')) {
      return {
        isValid: true,
        apiProvider: operatorId === 'auto' ? 'redbus' : operatorId,
        confidence: 95,
        details: {
          pnr: cleanPNR,
          passengerName: additionalData.passengerName || "John Doe",
          from: "Mumbai Central",
          to: "Pune Station",
          departureDate: "2024-01-20",
          departureTime: "14:30",
          busOperator: BUS_OPERATORS.find(op => op.id === (operatorId === 'auto' ? 'redbus' : operatorId))?.name || "RedBus",
          seatNumber: "A1",
          boardingPoint: "Mumbai Central Bus Stand",
          droppingPoint: "Pune Railway Station",
          ticketPrice: 450,
          ticketStatus: "CONFIRMED",
          phoneNumber: additionalData.phoneNumber || "+91-9876543210"
        }
      };
    }

    // Simulate different failure cases
    if (cleanPNR.includes('CANCEL')) {
      return {
        isValid: false,
        error: "Ticket has been cancelled",
        apiProvider: operatorId === 'auto' ? 'detected' : operatorId
      };
    }

    if (cleanPNR.includes('EXPIRED')) {
      return {
        isValid: false,
        error: "Ticket has expired or journey date has passed",
        apiProvider: operatorId === 'auto' ? 'detected' : operatorId
      };
    }
    ...
    return {
      isValid: false,
      error: "PNR not found in operator database",
